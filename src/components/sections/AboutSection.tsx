            ...
            <Container>
                ...
                <div className="grid md:grid-cols-2 gap-12 items-center mb-10 text-left">
                    <div className="space-y-4 text-left">
                        <p className="text-[14px] md:text-xl text-slate-800 dark:text-white font-bold leading-tight uppercase tracking-tighter text-left">
                            {t("Center of Excellence in Urology & Men's Health")}
                        </p>
                        <div className="space-y-4 text-slate-500 dark:text-slate-400 text-xs md:text-base leading-relaxed font-medium text-left">
                        ...
                        <Link href={lang === 'EN' ? '/urologist/dr.niti' : `/urologist/dr.niti?lang=${lang.toLowerCase()}`}>
                            <GradientButton
                                variant="outline"
                                className="px-8 py-3.5 rounded-xl text-[8px] md:text-[10px] text-left"
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left mb-16">
                    {ABOUT_SPECIALTIES.map((item, index) => (
                        <div key={index} className="group flex bg-linear-to-br from-white/80 via-white/50 to-indigo-50/20 dark:bg-none dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-200/50 dark:border-white/10 transition-all duration-300 hover:border-amber-600/30 dark:hover:border-amber-500/40">
                            <div className="relative w-[102px] sm:w-40 shrink-0 overflow-hidden text-left"><Image src={item.image} alt={item.title} fill sizes="(max-width: 640px) 102px, 160px" className="object-cover transition-transform duration-700 group-hover:scale-110" /></div>
                            <div className="p-4 md:p-6 flex flex-col justify-center text-left">
                                <div className="flex items-center space-x-2 mb-2 text-left"><item.Icon size={14} className="text-amber-600 md:w-4 md:h-4" /><h3 className="text-[11px] md:text-sm font-black text-slate-900 dark:text-white uppercase text-left">{t(item.title)}</h3></div>
                                <p className="text-slate-500 dark:text-slate-400 text-[11px] md:text-sm leading-relaxed font-medium text-left">{t(item.desc)}</p>
                            </div>
                        </div>
                    ))}
                </div>
